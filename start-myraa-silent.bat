...
for /f "tokens=5" %%a in ('netstat -ano ^| findstr ":3010" ^| findstr "LISTENING" 2^>nul') do (
...
for /l %%i in (1,1,20) do (
    ...
    powershell -NoProfile -Command "try { $r = Invoke-WebRequest -Uri 'http://localhost:3010' -UseBasicParsing -TimeoutSec 2; if ($r.StatusCode -eq 200) { exit 0 } else { exit 1 } } catch { exit 1 }" >nul 2>&1
    if not errorlevel 1 (
