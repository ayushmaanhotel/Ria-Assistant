...
export class MyraaAudioSession {
  ...
  public async connect() {
    ...
    try {
      ...
      const voice = activeAssistant === "Ria" ? (settings.riaVoice || "Kore") : (settings.myraaVoice || "Aoede");
      const configPath = activeAssistant === "Ria" ? (settings.riaCustomConfigPath || "") : "";
      ...
      this.ws.onopen = async () => {
        ...
        try {
          ...
          // Obtain User Microphone layout with graceful fallback
          let stream: MediaStream;
          try {
            const micDeviceId = settings.micDeviceId;
            stream = await navigator.mediaDevices.getUserMedia({
              audio: micDeviceId ? { deviceId: { exact: micDeviceId } } : {
            ...
            console.warn("Exact mic constraints failed, trying basic audio stream:", micErr);
