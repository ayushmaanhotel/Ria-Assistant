...
@register("openYouTube")
def open_youtube(args: Dict[str, Any]) -> Dict[str, Any]:
    query = args.get("query") or args.get("search")
    if query:
        url = _build_search_url("youtube", str(query))
        open_url(url)
        return {"result": f"Opened YouTube search for '{query}' on real Windows browser.", "url": url}
    else:
        url = "https://www.youtube.com"
        open_url(url)
        return {"result": "Opened YouTube on real Windows browser.", "url": url}
